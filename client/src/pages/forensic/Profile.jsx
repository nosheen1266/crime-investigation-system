...
export default function ForensicProfile() {
  ...
  useEffect(() => {
    axiosInstance.get('/api/forensic/profile').then(r => setProfile(r.data.data)).catch(() => toast.error('Failed'));
  }, []);
  ...
  const changePassword = async (e) => {
    ...
    setLoading(true);
    try {
      await axiosInstance.put('/api/forensic/change-password', { current_password: passForm.current_password, new_password: passForm.new_password });
  ...
  return (
    <div className="min-h-screen bg-navy-900 flex">
      ...
      <div className="flex-1 p-8 overflow-auto">
        <h1 className="text-2xl font-bold text-white mb-6">Forensic Profile</h1>
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="glass-card p-6">
            <h2 className="text-white font-semibold mb-4">Profile Info</h2>
            {profile && (
              <div className="space-y-2 text-sm">
                {[['Name', profile.full_name], ['Lab ID', profile.lab_id], ['Specialization', profile.specialization], ['Email', profile.email]].map(([k, v]) => (
