...
export default function ForensicLogin() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await axios.post('https://trace-dreamily-zap.ngrok-free.dev/api/auth/forensic/login', form, { withCredentials: true });
      if (res.data.success) {
        login(res.data.accessToken, res.data.user);
        toast.success('Welcome Forensic Officer!');
    ...
    <div className="min-h-screen bg-navy-900 flex items-center justify-center p-4">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="glass-card p-8 w-full max-w-md">
        <div className="text-center mb-8">
          <FlaskConical className="w-12 h-12 text-neon-blue mx-auto mb-3" />
          <h1 className="text-2xl font-bold text-white">Forensic Portal</h1>
          <p className="text-gray-400 mt-1">Forensic Lab Access</p>
        </div>
